...
```bash
docker compose exec backend uv run python manage.py enroll_staff_mfa reviewer@example.com
```
...
```bash
docker compose exec backend uv run python manage.py enroll_staff_mfa reviewer@example.com --replace
```
