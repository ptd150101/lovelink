...
def normalize_profile_image(bucket: str, temporary_key: str) -> dict:
    ...
    thumbnail.thumbnail((480, 600), Image.Resampling.LANCZOS)
    thumb = BytesIO()
    thumbnail.save(thumb, "WEBP", quality=82, method=6)

    put_object_bytes(bucket, object_key, full.getvalue(), "image/webp", public=True)
    put_object_bytes(bucket, thumbnail_key, thumb.getvalue(), "image/webp", public=True)
...
def normalize_private_image(bucket: str, temporary_key: str) -> dict:
    ...
    put_object_bytes(bucket, object_key, output.getvalue(), "image/webp", public=False)
    if object_key != temporary_key:
        delete_object(bucket, temporary_key)
    return {"object_key": object_key, "mime_type": "image/webp", "file_size": len(output.getvalue())}
