...
def profile_completion(profile):
    score = sum(bool(getattr(profile, field)) for field in REQUIRED_FIELDS)
    score += (
        bool(profile.looking_for)
        + bool(profile.interests.exists())
        + bool(profile.photos.exists())
    )
...
def validate_publish(profile):
    ...
    if age_from_birth_date(profile.birth_date) < 18:
        errors["birth_date"] = "Bạn phải từ 18 tuổi."
    if not profile.photos.filter(moderation_status="approved").exists():
        errors["photos"] = "Cần ít nhất một ảnh."
    if len(profile.bio.strip()) < 50:
        errors["bio"] = "Giới thiệu cần ít nhất 50 ký tự."
    return errors
...
def presence_for_viewer(target_user, viewer):
    ...
    if not users_are_connected(viewer, target_user):
        return None

    preferences = getattr(target_user, "preferences", None)
    if preferences is not None and not preferences.show_online_status:
        return None

    last_seen = target_user.last_seen_at
    if not last_seen:
        return {"status": "offline"}
    seconds = max(0, (timezone.now() - last_seen).total_seconds())
    if seconds <= 120:
        return {"status": "online"}
    if seconds <= 24 * 60 * 60:
        return {"status": "recently_active"}
    return {"status": "offline"}
