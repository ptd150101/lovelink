...
def send_message(*, conversation, user, text, client_message_id):
    membership = (
        ...
        .select_related("conversation__connection_request")
        .first()
    )
    if not membership:
    ...
    if not other or are_blocked(user, other.user):
        raise PermissionError("Blocked")
    if conversation.connection_request.status != "accepted":
    ...
    try:
        ...
        message = Message.objects.get(
            sender=user, client_message_id=client_message_id
        )

    payload = json_safe(MessageSerializer(message).data)
    ...
    push_notification(
        ...
        title=f"Tin nhắn từ {user.profile.display_name}",
        body=text[:120],
        actor=user,
        entity=conversation,
    )
