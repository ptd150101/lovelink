...
ALLOWED_HOSTS = [
    x.strip()
    for x in env("DJANGO_ALLOWED_HOSTS", "localhost,127.0.0.1").split(",")
    if x.strip()
]
...
CSRF_TRUSTED_ORIGINS = [
    x.strip()
    for x in env("CSRF_TRUSTED_ORIGINS", APP_URL).split(",")
    if x.strip()
]
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        "intro": "20/day",
        "message": "120/min",
        "phone_otp_send": "10/hour",
        "phone_otp_verify": "30/hour",
    },
}
...
CELERY_BEAT_SCHEDULE = {
    "expire-connection-requests": {
        "task": "apps.connections.tasks.expire_connection_requests",
        "schedule": 3600,
    },
    "expire-ringing-calls": {
        "task": "apps.calls.tasks.expire_ringing_calls",
        "schedule": 60,
    },
    "purge-verification-evidence": {
        "task": "apps.verification.tasks.purge_expired_evidence",
        "schedule": 86400,
    },
    "finalize-account-deletions": {
        "task": "apps.accounts.tasks.finalize_account_deletions",
        "schedule": 86400,
    },
}
...
PHONE_DEFAULT_COUNTRY_CODE = env("PHONE_DEFAULT_COUNTRY_CODE", "84")
PHONE_OTP_TTL_SECONDS = int(env("PHONE_OTP_TTL_SECONDS", "300"))
PHONE_OTP_RESEND_SECONDS = int(env("PHONE_OTP_RESEND_SECONDS", "60"))
PHONE_OTP_DAILY_LIMIT = int(env("PHONE_OTP_DAILY_LIMIT", "5"))
PHONE_OTP_MAX_ATTEMPTS = int(env("PHONE_OTP_MAX_ATTEMPTS", "5"))
PHONE_OTP_FIXED_CODE = env("PHONE_OTP_FIXED_CODE") if DEBUG else ""
...
S3_VERIFICATION_BUCKET = env(
    "S3_VERIFICATION_BUCKET", "verification-evidence-private"
)
